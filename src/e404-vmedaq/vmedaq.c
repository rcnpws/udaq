...
int main(int argc, char *argv[]){
 ...
 // unsigned short match_window_width = 4000/t_clock;     // from 1 to 4095 (used up to 2015.6.10 20:15)
  ...
  if(argc>1 && !strcmp(argv[1],"-h")){
    ...
    fprintf(stdout, "Usage: %s [-h] [-v V1190 module_numbers...] [-m MADC32 modules numbers ... ] file_name \n", argv[0]);
  ...
  while(argc>0){
    if(!strcmp(argv[0],"-i")){
    ...
    }else if(!strcmp(argv[0],"-v") || !strcmp(argv[0],"-v1190")){
      v1190_flag = 1;
      madc32_flag = 0;
    }else if(!strcmp(argv[0],"-m") || !strcmp(argv[0],"-madc32")){
      v1190_flag = 0;
      madc32_flag = 1;
      ...
      if(v1190_flag){
		  if(geo_flag==0){
			v1190_module_numbers[v1190_n_modules] = atoi(argv[0]);
			v1190_module_geo[v1190_n_modules] = atoi(argv[0]);
			v1190_n_modules++;
		  }
		  else{
			v1190_module_geo[v1190_n_modules-1] = atoi(argv[0]);
			geo_flag=0;
		  }
      }else if(madc32_flag){
		  if(geo_flag==0){
			madc32_module_numbers[madc32_n_modules] = atoi(argv[0]);
			madc32_module_geo[madc32_n_modules] = atoi(argv[0]);
			madc32_n_modules++;
		  }
		  else{
			madc32_module_geo[madc32_n_modules-1] = atoi(argv[0]);
			geo_flag=0;
		  }
      }
    }else if(argv[0][0]=='-'){
