/*
  tcp_write.c ... Write buffer to Router
  Copyright (C) 1995  A. Tamii
  Author:   A. Tamii
  Facility: Department of Physics, Kyoto University 
	    & Research Center for Nuclear Physics
  Created:  30-OCT-1996 by A. Tamii (router_write.c for Solaris 2.3)
  Ver 2.1   20-MAY-2000 by A. Tamii (Add sleep)
  Ver 2.2   25-MAY-2000 by A. Tamii (Add -b option)
  Ver 3.0   04-JUL-2014 by A. Tamii (tcp_write.c for GR-VDC DAQ with VME)
*/
...
char title[] = "TCP Write Ver 3.0 04-JUL-2014";
#if 0
...
void usage(){
  fprintf(stderr, "%s\n", title);
  fprintf(stderr, "  ... write data to router via TCP/IP\n", filename);
  ...
  fprintf(stderr, "       -b size .. write block size in kbytes (default = %d)\n",
	  BUFSIZE/1024);
     ...
     char   *argv[];
{
  ...
  /* Make Connection to Router via TCP*/
