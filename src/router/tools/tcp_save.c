/*
  ...
  Created:  18-FEB-1995 by A. Tamii (for Solaris 2.3) 
            ...
            10-NOV-2014 ver 3.1  by A. Tamii   (replaced write to fwrite for saving a file)
*/
...
static char title[] = "TCP Save Ver 3.1 10-NOV-2014";
#if 0
  ...
  ptr  = (unsigned char*)&info2;
	...
	for(i=1;;i++){
		...
		if(bufsize<0){
			...
			fwrite(buf, sizeof(unsigned char), bufsize, fd_write);
			if(bufsize<BUFSIZE)
				fwrite(zero, sizeof(unsigned char), BUFSIZE-bufsize, fd_write);
#else
			fwrite(buf, sizeof(unsigned char), bufsize, fd_write);    /* write data */
#endif
			/* free buffer */
			free(buf);
			buf = (char*)NULL;
