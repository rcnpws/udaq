/*
  ...
  Version 3.0  27-JAN-2018 by A. Tamii (accept the data transfer of .bld file)
*/
...
char title[] = "router_tcp ver 3.0 27-JAN-2018";
...
int send_data_from_router(int fd_read){
  char  *buf;
	int   end_flag;
  int   count;
	long  size;
...
int send_data_from_file(FILE* fd){
  char  *buf;
	int   end_flag;
  int   count;
	long  bsize, size;
  ...
  int   reply;
	...
	end_flag = 0;
	for(i=0; !end_flag;i++){
		size = fread(&header, 1, sizeof(bld1_header_t), fd);
		if(size==0) break;
		if(size!=sizeof(bld1_header_t)){
			msgout(ErrSever, "Incomplete header. Size = %d", size);
			return -1;
		}
#if 0
		for(i=0; i<sizeof(bld1_header_t); i++){
			if(i%16==0) fprintf(stderr, "\n");
			fprintf(stderr, "%.2x ", ((char*)&header)[i]);
		}
#endif
		bsize = ntohl(header.bsize);
		if(bsize<0 || MAX_BUF_SIZE<bsize){
			msgout(ErrSever, "Illegal block size. Size = %d (0x%x)", bsize, bsize);
			return -1;
		}
		// fprintf(stderr, "bsize = %d (0x%x).\n", bsize, bsize);
		size = fread(buf, sizeof(char), bsize, fd);
		if(size!=sizeof(char)*bsize){
			msgout(ErrSever, "Incomplete data. Size = %d, Block-Size = %d", size, bsize);
			return -1;
		}

		/* send data via TCP/IP (variable length) */
		info3.blk_count = htonl(i);
		info3.blk_size  = htonl(sizeof(header)+size);
		if(write(sd, &info3, sizeof(info3))<0)     
			msgout(ErrSever, "Error in write(): %s", strerror(errno));
...
void send_data()
{
  ...
  strcat(str, name);
	...
	case ROUTER_INET_VERSION_2:
		...
		if(!strncmp(info1.router,".bld",4)){
			file_flag = 1;
			sprintf(filename, "%s/data/run%d.bld", getenv("HOME"), info1.stream);
	...
	if(file_flag){
		fd = fopen(filename, "r");
		if(fd==(FILE*)NULL){
			msgout(ErrSever, "Error in opening file '%s'.", filename);
			return;
		}
		send_data_from_file(fd);
		fclose(fd);
	}else{
		send_data_from_router(fd_read);
		router_disconnect(fd_read);
	}
}
