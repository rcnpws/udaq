  ...
  ptr  = (char*)&info2;
	...
	for(i=1;;i++){
      ...
      if(count>bufsize){
		...
		/* read a block */
		count = 0;
fprintf(stderr, "Enter: reading a block\n");
		while(1){
...
fprintf(stderr, "Exit: reading a block count=%d\n", count);
		if(size==0){
...
if(count!=0x4000)
  fprintf(stderr, "size = %x\n", count);
		...
		/* send an acknowledge */
fprintf(stderr, "Enter: Sending acknowledge\n");
		reply = ntohl(0);
		if(write(sd, &reply, sizeof(reply))<0){
			close(sd);
			msgout(ErrSever, "Error in write(). %s", strerror(errno));
...
fprintf(stderr, "Exit: Sending acknowledge\n");
	}
