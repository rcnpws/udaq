...
typedef struct bld1_header{  /* all components are defined by the network byte order */
  ...
  unsigned int    prev_pos1; /* absolute prev buffer position in byte (upper)*/
  unsigned int    prev_pos2; /*                                       (lower)*/
  unsigned int    next_pos1; /* absolute prev buffer position in byte (upper)*/
  unsigned int    next_pos2; /*                                       (lower)*/
} bld1_header_t;
