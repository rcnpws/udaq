...
void send_data()
{
  ...
  /* check the version */
	if(ntohl(info1.ver) != ROUTER_INET_VERSION_1){
  ...
  info2.res1 = info2.res2 = 0;
	...
	for(i=0;;i++){
		...
		if(write(sd, buf, blksize)<0)        /* write data via TCP/IP */
			perror("write");                   /* Always send 'blksize' Bytes */
		count = 0;
		...
		reply = ntohl(reply);
		if(reply!=0){
			fprintf(stderr, "Error reply from the target host(%d).\n", reply);
		}
	}
