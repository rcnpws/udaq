...
int main(int argc, char *argv[]){
  ...
  if(argc>1 && !strcmp(argv[1],"-h")){
    fprintf(stderr, "%s ... read v1190 read status TDC header and trailer.\n", argv[0]);
  ...
  fprintf(stderr, "micro code read: V1190_OPECODE_READ_HEAD_TRAILER\n");
  ret = v1190_micro_handshake_read(v1190, V1190_OPECODE_READ_HEAD_TRAILER, &wdata);
  if(ret!=0){
    fprintf(stderr, "return code from v1190_micro_handshake_read: %d\n", ret);
  }
