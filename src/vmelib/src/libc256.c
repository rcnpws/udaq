...
void c256_reset(struct K_REG *k2917,int c){
  ...
  nfa.n = 3;
  nfa.c = c;
  CNAF(k2917,nfa.c,nfa.n,nfa.a,nfa.f,&nfa.data,&q,&x);
  nfa.n = 4;
  CNAF(k2917,nfa.c,nfa.n,nfa.a,nfa.f,&nfa.data,&q,&x);
  nfa.n = 7;
  CNAF(k2917,nfa.c,nfa.n,nfa.a,nfa.f,&nfa.data,&q,&x);
...
void c256_read_dbuf_mode(struct K_REG *k2917, unsigned short sbuf[], int ch) /* ch: 1 or 2 */
{ 
  int i,j,status;
  unsigned long q,x;
  int cur_ptr=0;
  ...
  for(j=spin_up;j<=spin_down;j++){
    ...
    for(i=0;i<16;i++){
      nfa.a=i;
      status = CNAF(k2917,nfa.c,nfa.n,nfa.a,nfa.f,&nfa.data,&q,&x);
