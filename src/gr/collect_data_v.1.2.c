...
#define MAX_SCL_NUM 2                 /* Number of VTS801 Scaler        */
...
void *collect_data(void *arg)
{
  ...
  Vts801 scl[MAX_SCL_NUM];            /* VTS801 Scaler                  */
  ...
  /* Open VME device(A32D32) for VTS801     */
  vmedev3 = Open(VTS801_MODE,O_RDWR,O_EXCL);
  ...
  for( scl_addr = VTS801_ADDR,i=0;i<MAX_SCL_NUM;i++){

    /* Initialize each VTS801 module */
    scl[i] = vts801_map(vmedev3,scl_addr);
    scl_addr += 0x10000; 
  }
  ...
  for(;;){
	...
	if(SINGLE_BUF_OVF(i_buf)){             /* Single buffer OVF                  */
	  ...
	  /* For Kaizu VTS801 scalers */

	  scl_word = vts801_read(scl[i_buf],sbuf);
	  vts801_reset(scl[i_buf]);
	  ...
	  for(buf_loop=0; buf_loop < 2; buf_loop++){
	    ...
	    scl_word = vts801_read(scl[p_buf],sbuf);
	    vts801_reset(scl[p_buf]);
